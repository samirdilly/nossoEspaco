Brewer.Venda = (function(){
	...
	function onValorFreteAlterado(evento){
		this.valorFrete = Brewer.recuperarValor($(evento.target).val());//tira a formatação da máscara para fazer cálculos		
	}
	
	function onValorDescontoAlterado(evento){
		this.valorDesconto = Brewer.recuperarValor($(evento.target).val());//tira a formatação da máscara para fazer cálculos
	}
